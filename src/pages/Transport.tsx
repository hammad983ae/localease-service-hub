...
const Transport: React.FC = () => {
  ...
  // Check URL parameters for pre-selected options
  useEffect(() => {
    const urlServiceType = searchParams.get('serviceType');
    const urlType = searchParams.get('type');
    
    if (urlServiceType && urlType) {
      // Both service type and selection type are provided
      if (urlType === 'quote' || urlType === 'supplier') {
        setSelectedService(urlServiceType);
        setSelectedType(urlType as 'quote' | 'supplier');
        console.log('🔍 Transport page: Found URL parameters - serviceType:', urlServiceType, 'type:', urlType);
      }
    } else if (urlServiceType) {
      // Only service type is provided, show type selection
      setSelectedService(urlServiceType);
      setIsServiceTypeModalOpen(true);
      console.log('🔍 Transport page: Found URL parameter serviceType:', urlServiceType);
    }
  }, [searchParams]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <ServiceTypeModal
        ...
        serviceType={selectedService ? transportServices.find(s => s.id === selectedService)?.title : undefined}
      />
    </div>
  );
};
